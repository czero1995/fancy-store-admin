<template>
  <div class="container p10 flex1">
    ...
    <Table :mainData="mainData">
      ...
      <el-table-column width="200" align="center" label="操作" v-permission="[1]">
...
<script>
import { apiGetBanner,apiEditBannerItem,apiAddBannerItem,apiDeleteBannerItem } from "@/api/product/banner";
...
export default {
  ...
  methods: {
    async getMainData() {
     let res = await apiGetBanner(this.title);
      this.mainData = res.data.data;
    },

    async editRowData(data, index) {
      console.log('data: ', data);
      let res = await apiEditBannerItem(data.uid, data)
      if (res.data.code == 0) {
        this.editSuccess(index,data);
      }
    },
    async addRowData(data) {
      let res = await apiAddBannerItem(data)
      if (res.data.code == 0) {
    ...
    async onDelete(rowIndex, rowData) {
      let res = await apiDeleteBannerItem(rowData.uid)
      if(res.data.code == 0){
